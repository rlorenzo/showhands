#!/usr/bin/env bash
# Deploy origin/main to the droplet (small-apps). Invoked by CI as root through
...
# run the deploy and never get a shell. The wrapper guarantees this file is
# current before it runs; see bootstrap.sh for why that indirection exists.
# Also runnable by hand: sudo deploy/deploy.sh
set -euo pipefail
